...
int add_var(SymTable* st, char* s, int line,int scope) {
    ...
    st->t[st->size].func_pointer = NULL;
...
AST* get_func_pointer(SymTable* st, int i) {
    return st->t[i].func_pointer;
}

void insert_func_pointer(SymTable* st, int i, AST* ptr) {
    st->t[i].func_pointer = ptr;
}
